...
class TestRCAVisualizerPrefix {
   ...
   void testNoVisualizerPrefix() throws Exception {
      ...
      assertEquals("myModule§de.package.TestClazzA_testSomething", action.get(0).getDisplayName());
      assertEquals("otherModule§net.package.TestClazzB_testSomethingElse", action.get(1).getDisplayName());
   }

   @Test
   void testSimpleVisualizerPrefix() throws Exception {
      ...
      List<RCAVisualizationAction> action = captor.getAllValues();
      assertEquals("TestClazzA_testSomething", action.get(0).getDisplayName());
      assertEquals("TestClazzB_testSomethingElse", action.get(1).getDisplayName());
   }

   @Test
   void testModuleVisualizerPrefix() throws Exception {
      ...
      List<RCAVisualizationAction> action = captor.getAllValues();
      assertEquals("de.package.TestClazzA_testSomething", action.get(0).getDisplayName());
      assertEquals("net.package.TestClazzB_testSomethingElse", action.get(1).getDisplayName());
   }

   private Run createChangefileActions(final File changefile) throws Exception {
...
//            final String name = testcases.getKey() + "_" + change.getMethod();
            ...
            rcaVisualizer.createRCAAction(longestPrefix, testcases, change, metadata);
         }
      }
      return run;
   }
