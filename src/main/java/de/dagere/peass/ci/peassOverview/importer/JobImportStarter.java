...
public class JobImportStarter implements Callable<Void> {
   ...
   @Option(names = { "-projectName",
         "--projectName" }, description = "Name of the project (if left out, the name of the workspace folder, which is the Jenkins job name, is used", required = false)
   ...
   public Void call() throws Exception {
      OneJobImporter oneJobImporter;
      if (projectName != null) {
         oneJobImporter = new OneJobImporter(projectResultsFolder, workspaceFolder, url, authentication, projectName, timeout);
      } else {
         oneJobImporter = new OneJobImporter(projectResultsFolder, workspaceFolder, url, authentication, projectResultsFolder.getName(), timeout);
      }
